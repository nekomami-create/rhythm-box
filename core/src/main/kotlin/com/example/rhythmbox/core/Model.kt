...
data class TrackSetting(
    ...
    /**
     * 左右の位置（定位）。-1 で左端、0 で中央（既定）、1 で右端。
     * 古い保存データは 0 として読めるので、今までと同じ聞こえ方になる。
     */
    val pan: Float = 0f,
)
...
enum class ChordStyle(val label: String) {
     ...
     *
     * 画面に出す名前は「高速」。「高速アルペジオ」だと 7 文字あって、
     * 弾き方の 5 つを 1 行に並べたときに右へはみ出す。上へ / 下へ / 上下 と
     * 並んでいるので、回すものだということは並びから分かる。
     */
    CHIP_ARPEGGIO("高速"),
    ;
